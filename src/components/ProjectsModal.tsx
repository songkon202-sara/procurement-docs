...
/**
 * Which cases show by default depends on the viewer's role: an officer's own drafting
 * workspace vs. an auditor's/approver's review queue for everyone else's submissions.
 * A user with procurement_officer wins even if they also hold another role, since "my own
 * work" is the more common thing to land on; pure auditor/approver accounts get their queue.
 */
function defaultView(roles: string[] | undefined): { query: string; label: string } {
  if (!roles?.includes('procurement_officer') && roles?.includes('auditor')) {
    return { query: 'status=submitted', label: 'รอตรวจสอบ (ทั้งหน่วยงาน)' };
  }
  if (!roles?.includes('procurement_officer') && roles?.includes('approver')) {
    return { query: 'status=under_review', label: 'รออนุมัติ (ทั้งหน่วยงาน)' };
  }
  return { query: 'mine=true', label: 'โครงการของฉัน' };
}
...
export function ProjectsModal() {
  ...
  const view = defaultView(user?.roles);
  const canCreate = user?.roles.includes('procurement_officer') ?? false;

  const refresh = async () => {
    setListError(null);
    try {
      const rows = await api.get<ApiCase[]>(`/cases?${view.query}`);
      setCases(rows);
    } catch (err) {
    ...
    >
      ...
      >
        <div style={{ padding: '16px 20px', borderBottom: '1px solid #e2e7ee', display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 16, fontWeight: 700, color: '#15293f' }}>{view.label}</div>
            <div style={{ fontSize: 12, color: '#7a8794' }}>{cases.length} โครงการ · กำลังทำ: {curCaseName}</div>
          </div>
        ...
        <div style={{ overflowY: 'auto', padding: '10px 16px', flex: 1 }}>
          ...
          {!loading && !listError && sorted.length === 0 && (
            <div style={{ textAlign: 'center', color: '#9aa6b3', fontSize: 13, padding: '24px 12px' }}>
              {canCreate ? 'ยังไม่มีโครงการที่บันทึกไว้ — กด "บันทึกโครงการปัจจุบัน" เพื่อเก็บชุดข้อมูลนี้ และเปิดใช้ภายหลังได้' : 'ไม่มีเรื่องรอดำเนินการในขณะนี้'}
            </div>
          )}
          {sorted.map((c) => {
